...
> **AI（アプリ）で音源を取り込む際は必要なモジュールをインストールしてください:** 機能ごとに追加モジュールが要ります。**URL取り込み**（YouTube等）= `yt-dlp` と `ffmpeg`（macOS は `brew install yt-dlp ffmpeg`）／**楽器分離** = Demucs用venv（`.venv-demucs`）／**多声検出**（和音・伴奏入り音源）= basic-pitch用venv（`.venv312`・Python 3.12）。入れ方は [エンジン README のセットアップ](spike/ear-pipeline/README.md#2-セットアップ) 参照。未インストールの機能は正直にエラー/フォールバックします。
...
> **精度の正直な現在地と原理的な限界**: 和音入りTAB譜は出力可能（課題曲の和音縦積み再現率 92.6%・プロ録音10曲平均 77.9% / F1 0.690）。ただし「プロの手書きTABと同じ完成度の全自動生成」は倍音縮退・弦割り当て不定性という物理的な壁により現技術では到達不能——理由と実測根拠は [docs/tab-limitations.md](docs/tab-limitations.md) にまとめてある（最新公開モデルMuScriptorとの比較含む）。現実的な路線は「たたき台TAB＋人間の仕上げ」。
...
| 7. デスクトップアプリ | **MVP動作（楽器選択対応）** | [app/](app/)（Electron）。ドラッグ&ドロップまたは**YouTube等のURL貼付**（yt-dlpローカル実行・私的利用前提・F-006裁定変更2026-07-23）→**採譜前のモード確認画面**（⚡スピード重視=TAB譜/五線譜から1形式だけ最速生成、📚楽譜の数重視=全形式。2026-07-27）→**Demucsで楽器分離→ギター/ピアノ/ボーカル/ベースを選んで採譜**（同一楽器の複数トラックは1つに混合される制限あり #130）→PDFアプリ内表示（ギターはTAB/五線譜切替・GP風リズム表記/休符/和音囲み #127）→エクスポート。**BPM/拍子/キーの任意指定**（分かる人は指定・未指定は自動）。処理画面は宇宙背景。解析ビュー（信頼度ハイライト＋波形）あり。楽譜エディタ・スペクトログラムは今後（#125） |
