...
- **同一楽器の複数トラック（ツインギター・ピアノ連弾等）は分離できない**（#130）:
  Demucs は楽器クラス単位の分離のため 1 つのステムに混合される。混合ステムは
  ポリフォニー高判定になり、ギター TAB は各瞬間の最も目立つ音を拾った
  単旋律（リード/バッキング混在の 1 本の線）になる。コード帯がバッキングの
  和声情報を補足する。楽器インスタンス分離は現行研究でも未解決級（将来の R&D 枠 #114）
