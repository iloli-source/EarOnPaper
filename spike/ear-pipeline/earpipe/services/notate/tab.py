...
# 低フレット優遇の撤廃(#144運指較正 2026-07-25): 本人運指つき正解データ(夢見る99打)で
# HEIGHT=0.05/FRET=0.02 は運指一致3/99だった。低フレットバイアスが「1つのポジション帯に
# 留まり形をスライドする」ロックの定石(DS-04)と衝突するため。0/0で62/99に改善
# (ポジションのタイはDP走査順=低ポジション側が決定的に選ばれ、単音の弦内タイは
# 貪欲の最低フレット選好が残るため退行しない。旗艦G5/バレー/開放弦選好sanity全緑)。
_HEIGHT_COST = 0.0
_FRET_COST = 0.0
...
# ボーナス強度は本人運指つき正解データ(夢見る99打)で較正(2026-07-25):
# POWER 0.35では「ポジション据え置きの混成ヴォイシング」が形を壊して勝つ
# (移動コスト>ボーナス)。2.0で運指一致62→68/99。低弦ルート選好も実験したが
# 全曲では逆効果(68→49・本人はC#5を5弦4Fで弾く)のため不採用。
_IDIOM_POWER = 2.0     # 隣接弦で+7半音 = root+5th(パワーコード形)
