...
| F-024 | **連桁・符尾の自動整理** | 1 | ✅ | `score.py` の music21 `makeNotation` が拍単位で連桁(beam)・符尾(stem)を自動整理(Verovio以前のMusicXML段階で確定)。独自エンジンは持たない(makeNotationが正しく処理・KISS)。`test_beaming.py` で回帰固定 |
...
| F-005 | マイク/ライン録音入力 | 2 | ✅ | `pipeline.py` `record` サブコマンド(録音→wav保存→任意で採譜)。録音は任意依存 sounddevice(未導入時は導入方法を提示して失敗)。`test_record.py` で境界検証 |
...
- **#109 結線(偽成功の解消)**: ✅✅ 「実装済み・ユニット緑だが pipeline から未到達(孤立)」を機械ゲート(#111)で85件凍結→**0件(完全消化)**。
  - B-2a: 移動ド/度数/Nashville を `transcribe --analysis`(F-091/F-100)
  - B-2 汎用エミッタ(`--emit KEY`, 自動発見レジストリ): 29モジュールを結線。ドラム検出/譜(F-018/F-036)・音質診断(F-002)・楽器分類(F-015)・強弱/ペダル/装飾(F-016/F-102/F-045)・声部分離(F-023)・移調/簡略化/音階整形(F-025)・簡譜/リード/度数変換・楽器プロファイル(F-079)・運指・試聴/サウンドフォント(F-054)・ビジュアルカード・歌詞整列・MuseScore下書き/受渡(F-093)・小節補正/MIDI整理・区間切出/genai前処理 等。既定の五線譜/MIDI/PDF/TAB 出力は不変(オプトイン副次出力)。各モジュールに自己検証テスト付き。
  - #113: gp5 の非表現音価クラッシュを修正し `--format gp5` に再結線。
  - 残subcommand型: `chunk`(長尺分割 F-004)・`diff`(譜面差分)・`compare`(AIの耳比較)を CLI サブコマンド化。
  - **孤立0達成**。空 allowlist をベースラインに、以後の新規孤立は即CI fail。
- **残: PDF/SVG視覚化の一部**（簡譜/リードPDFは `--emit textpdf` で結線済。度数/移動ドはテキスト。五線譜/TABは従来どおり視覚化済み）
- **E. 編集UI/アプリ**（最大の残塊）: F-038/039/041/042/043/046・波形/スペクトログラム（Electron本格実装）
