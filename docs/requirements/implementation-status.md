...
- **C/D. 解析・整譜・記譜形式**: ✅ batch1(#66-75)・batch2(#76-81)・batch3(#82/83/84/87＝F-099 LLM出力/F-016強弱/F-018ドラム/F-060移調) 完了。**全体テスト664緑**。batch3b(F-037記譜変換/F-095密度簡略化)は研究後追走中。
- **残: 記譜レンダリング**（简谱/リードシート/度数/移動ドはテキスト出力のみ→PDF/SVG描画が未。五線譜/TABのみ視覚化済み）
