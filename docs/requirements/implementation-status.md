...
> **2026-07-23 セッション更新（アプリ＋エンジン）:**
> - **アプリでTAB譜出力**＋**楽器分離→楽器選択UI**（Demucsで分離→ギター/ピアノ/ボーカル/ベースを選び、その楽器だけ採譜）。ギターは五線譜/TAB切替。分離は1回・採譜はオンデマンド。
> - **6-stem分離（htdemucs_6s）でギター/ピアノを個別分離**（F-003強化）。分離のみの `separate` サブコマンド新設。※ピアノは音漏れ多く実験的品質。
> - **BPM・拍子・キーの任意上書き**（F-012/F-013関連）: CLI `--bpm`/`--bpm-range`/`--beat`/`--key`＋`--mode`、アプリの詳細設定。未指定は自動検出。カエルの歌の3拍子誤検出は `--beat 4/4` で矯正可。
> - **CJKタイトルの豆腐化修正**（TAB=cairosvg・五線譜=Verovio 双方。韓国語含む pan-Unicode フォント先頭化）。
> - **コード押さえ図を反時計回り90度回転**（見やすさ・コード帯）。
> - **リズム根治(F-010/F-021関連・#118)**: 単純曲の三連符誤爆は `estimate_grid` の格子判別ではなく、mono検出(pYIN)が**同音の反復8分を1音にマージして取りこぼす**のが主因と切り分け。`ear/mono.py` に**RMSエネルギー再アタックによる分割**を追加し根治(かえる: 三連10→0・当時の記録で全1201テスト緑・estimate_grid不変でRomanze退行なし)。詳細は [accuracy-improvement-roadmap.md](../research/accuracy-improvement-roadmap.md)。
> - 残課題（未反映）: **3/4拍子の誤判定(estimate_meter・別軸/調律済/--beat 4/4回避)**、多声のコーパス誤三連(格子判別)、ギターTABの密・跳ね、ピアノ分離品質、編集UI層、記譜描画（简谱等のPDF化）、コード譜専用ビュー。
...
| F-006 | 動画サイトURL直接入力 | 2 | ✅ | app `import-url`(yt-dlpローカル実行・#128)。裁定変更2026-07-23 |
...
| F-043 | 信頼度ハイライト | 2 | ✅ | `notate/analysis_view.py`＋`emitters/confview.py`(#121一部・97aeb6f)。GUI「解析」タブで低信頼=赤の校正ビュー表示 |
...
- **C/D/G. 解析・整譜・記譜・記譜描画・出力・練習ロジック**: ✅ batch1〜5(#66-108)で**エンジン/記譜のロジック系はほぼ全実装**(多声分離・歌詞同期・譜面差分・資産再利用・出力プロファイル・形式レジストリ・MuseScore連携・人手仕上げパッケージ・サウンドフォント試聴・区間採譜・解析ヒント・テンポ変更再生・バッチ・ドラム譜・楽器プロファイル・装飾音・鍵盤運指・ビジュアルカード・简谱/リードシートPDF描画 等)。**全体テスト1041緑（当時の記録・現行HEADの値ではない）**。
...
- **E. 編集UI/アプリ**: ✅ **Electron MVP動作(#61・E2E検証済)** — ドラッグ&ドロップ/URL取り込み(#128)→採譜→PDFアプリ内表示→PDF/MusicXML/MIDIエクスポート。解析ビュー(信頼度ハイライト＋波形・#121一部)実装済み。Playwright GUI E2Eで4受入条件を固定。**GUI E2Eが実バグ(sandbox既定でpreload失敗→window.earpipe未公開)を検出・修正**(偽成功の実例)。残: F-039楽譜エディタ(#125)・F-046アンドゥ・F-041スペクトログラム
