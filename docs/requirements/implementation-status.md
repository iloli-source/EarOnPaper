...
- **#109 結線(偽成功の解消)**: ✅ 「実装済み・ユニット緑だが pipeline から未到達(孤立)」を機械ゲート(#111)で85件凍結→**6件まで消化**。
  ...
  - 残6件(意図的保留・allowlist凍結): `diff_notes`(2譜面必須)・`run_compare`/`build_compare_command`(外部ツール)・`split_into_chunks`/`Chunk`(音声分割)=将来サブコマンド、`write_guitarpro`=producer欠陥(#113)。
...
| F-073 | 権利ガイダンス表示 | ⬜ 実装対象(コード) | 採譜物の配布/販売時の著作権注意をユーザーに示す教育UI。課金と無関係で**コード実装対象** |
