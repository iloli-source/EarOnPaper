...
> - **リズム根治(F-010/F-021関連・#118)**: 単純曲の三連符誤爆は `estimate_grid` の格子判別ではなく、mono検出(pYIN)が**同音の反復8分を1音にマージして取りこぼす**のが主因と切り分け。`ear/mono.py` に**RMSエネルギー再アタックによる分割**を追加し根治(かえる: 三連10→0・全1201テスト緑・estimate_grid不変でRomanze退行なし)。詳細は [accuracy-improvement-roadmap.md](../research/accuracy-improvement-roadmap.md)。
